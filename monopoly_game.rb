...
until exit_text(user_input)
  @players.each { |player|
    ...
    if @players.length == 1
       puts "#{@players.name} since you are the only player left in a game, you are the winner."
       user_input = 'Exit'
    end
...
# add the go space or you can add jail (these are not properties) - Done sort of
# with go space, if you pass the go, you have to put 200$ in players's cash - Done sort of
...
# HW -1/20/21
# When the person passed 0 element on board array they get 200$
#
# 1/28/21
...
# Jail is a double space - one visiting other is in jail - different rules for these two statuses
# Make a spacial method for jail (you will end their turn, not get paid for going around, ) - done
# put rail roads on the board
#
# 2/3/21
# divide modules.rb in to separate modules logically - done
# when you first land in jail, you don't have any option bu t trading and you should be able to end turn - have to do
# in your next turn,you have option to roll and if you roll double you will get out of jail - need to test
# Otherwise you can chose to pay 50 $ and get out of jail and roll and conitune play - need to test
...
# turn should be called over and over again for the same player ...you moved, now call turn to do next thing in the same turn - needs to be done
# bug: in same turn if user have to pay rent twice in case rolling double, rent is not an option
#pay rent got messed up
